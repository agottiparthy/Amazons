...
class BoardWidget extends Pad {
    ...
    private void drawGrid(Graphics2D g) {
        ...
        for (int row = 0; row <= Board.SIZE - 1; row++) {
            yCorner = row * SQUARE_SIDE;
            for (int col = 0; col <= Board.SIZE - 1; col++) {
                xCorner = col * SQUARE_SIDE;
                if (row % 2 == 0 && col % 2 == 0) {
                ...
                g.fillRect(xCorner, yCorner, SQUARE_SIDE, SQUARE_SIDE);
            }
        }
    }
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        for (int i = Board.SIZE - 1; i >= 0; i -= 1) {
            for (int j = 0; j < Board.SIZE; j++) {
                Piece piece = _board.get(sq(j, i));
                if (piece == BLACK) {
                    drawQueen(g, sq(j, i), BLACK);
                } else if (piece == WHITE) {
                    drawQueen(g, sq(j, i), WHITE);
                } else if (piece == SPEAR) {
                    drawSpear(g, sq(j, i));
                }
            }
    ...
    private void click(Square s) {
        ...
        numAdded++;
        if (numAdded == 3) {
            Move move = mv(sqs[0], sqs[1], sqs[2]);
    ...
    /** Denotes the first time we paint the grid. */
    private int count = 1;

    /** The command we pass in when clicking squares. */
    private String command = "";

    /** The different squares we click on to create a move. */
    private Square[] sqs = new Square[3];

    /** A counter to ensure 3 squares are clicked before a move is executed. */
